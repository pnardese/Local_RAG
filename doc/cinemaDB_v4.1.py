...
class DocumentToChromaDB:
    ...
    def process_documents(self, force_reprocess=False):
        ...
        # --- FIX: BATCH PROCESSING START ---
        if all_documents:
            # We set a batch size well below the 5461 limit (e.g., 2000 or 5000)
            BATCH_SIZE = 2000
            total_chunks = len(all_documents)
            
            # Optional: Add a specific progress bar for DB insertion
            db_status = st.empty()
            ...
            for i in range(0, total_chunks, BATCH_SIZE):
                ...
                db_status.text(f"Saving to Database: {i} to {end_idx} of {total_chunks} chunks...")
...
def main():
    ...
    with st.sidebar:
        ...
        llm_model = st.text_input("Ollama Model", "qwen3:4b")
