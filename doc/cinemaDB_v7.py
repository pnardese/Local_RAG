...
def main():
    ...
    with st.sidebar:
        ...
        embedding_model = "nomic-embed-text"  # Default value
        
        # --- TWO DIFFERENT MODELS INPUT ---
        embedding_model = st.text_input("1. Embedding Model (Search)", embedding_model, help="Used for indexing files. If you change this, you MUST Force Full Rescan.")
        chat_model = st.text_input("2. Chat Model (Answer)", chat_model, help="Used for talking to you.")
        
        # --- NEW: SOURCE NUMBER SLIDER ---
        st.divider()
        st.header("Search Parameters")
        num_results = st.slider("Number of Sources to Retrieve", min_value=1, max_value=20, value=8, help="How many document chunks to feed the AI.")
    ...
    if prompt := st.chat_input("Ask a question..."):
        ...
        with st.chat_message("assistant"):
            # --- UPDATED CALL: Passes num_results from sidebar ---
            result = doc_db.search_and_answer(prompt, chat_model=chat_model, n_results=num_results)
